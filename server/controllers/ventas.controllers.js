...
export const ListarVentas = async (req, res) => {
  try {
    const response = await sequelize.query(
      `SELECT v.id_venta, r.nombre_cliente, r.apellidos, pro.nombre_producto, v.descripcion, v.pago_confirmado, pro.precio_venta
      FROM "ventas" AS v 
      JOIN reservas AS r ON v.id_reserva = r.id_reserva
      JOIN productos AS pro ON r.id_producto = pro.id_producto
...
export const crearVenta = async (req, res) => {
  ...
  try {
    const { id_producto, id_reserva, id_trabajador, descripcion } = req.body;
